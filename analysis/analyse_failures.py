...
def evaluate():
    ...
    df_caam = pd.read_csv(f"analysis/stats/caam_0.csv", sep=",", index_col="basin")
    df_caam.index = [str(b).rjust(8,"0") for b in df_caam.index]
    df_enca2 = pd.read_csv(f"analysis/stats/{experiment}_2.csv", sep=",", index_col="basin")
    df_enca2.index = [str(b).rjust(8,"0") for b in df_enca2.index]
    df_enca3 = pd.read_csv(f"analysis/stats/{experiment}_3.csv", sep=",", index_col="basin")
    ...
    for i in range(len(encoded_features)):
        ...
        if i==0:
            ...
            df_prev = pd.read_csv(f"analysis/stats/{experiment}_{encoded_features[i-1]}.csv", sep=",", index_col="basin")
            df_prev.index = [str(b).rjust(8,"0") for b in df_prev.index]

        # load model
        df_model = pd.read_csv(f"analysis/stats/{experiment}_{encoded_features[i]}.csv", sep=",", index_col="basin")
