...
class CamelsTXT(Dataset):
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # assert(np.all(y.flatten()>=0))
        # assert(np.all(np.logical_not(np.isnan(y.flatten()))))
...
class CamelsH5(Dataset):
    ...
    def __getitem__(self, idx: int):
        if self.cache:
            x = self.x[idx]
            y = self.y[idx]
            #y_to_encode = self.y_to_encode[idx]
            ...
            with h5py.File(self.h5_file, 'r') as f:
                x = f["input_data"][idx]
                y = f["target_data"][idx]
                #y_to_encode = f["to_encode_target"][idx]
        ...
        x = torch.from_numpy(x.astype(np.float32))
        y = torch.from_numpy(y.astype(np.float32))
        # y_to_encode = torch.from_numpy(y_to_encode.astype(np.float32))
    ...
    def _preload_data(self):
        with h5py.File(self.h5_file, 'r') as f:
            x = f["input_data"][:]
            y = f["target_data"][:]
            # y_to_encode = f["to_encode_target"][:]
